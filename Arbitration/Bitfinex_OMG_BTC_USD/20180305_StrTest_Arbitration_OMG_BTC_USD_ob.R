  ...
  if(trade == 0){
    ...
    if(m[i,17] > s + scaling && trade_positive_difference == 1 && balance_avail > 100){
      ...
      avg_price_OMGBTC <- abs((temp_price_OMGBTC*temp_OMGBTC + deal/m[i,16]*m[i,8]) /
        (temp_OMGBTC + deal/m[i,16]))
        #(balance_used / 2) / abs(balance_OMGBTC)
      
      s <- m[i,17]
      
      # cat("DateTime: ", as.character(as.POSIXct(m[i,1], origin = "1970-01-01")),
      #     "Balance USD:",format(round(temp_usd,2), nsmall = 2),
      #     "Balance OMG/BTC:",format(round(temp_OMGBTC,2), nsmall = 2),
      #     "Balance OMG/USD:",format(round(temp_OMGUSD,2), nsmall = 2),"\n")
...
write.csv(res, "OMG_balance.csv", row.names = FALSE)
