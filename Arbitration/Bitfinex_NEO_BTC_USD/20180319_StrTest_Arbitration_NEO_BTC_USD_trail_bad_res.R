...
for(i in seq(11, nrow(m), by = 5)){
  ...
  if(trade == 0){
    ...
    if(BuyNEOBTC == 1){
      ...
      PL <- (PL_NEOBTC+PL_NEOUSD+PL_BTCUSD-commission*2)/balance_used*100;
      if(PL > max_pl) max_pl <- PL
      trail <- max_pl - trail_range
      if(PL < min_pl) min_pl <- PL
      
      if((PL>cond_close || PL<stop_loss || m[i,1]-start > stop_loss_dur
        ...
        if(BTCUSD_cond == 0){
          ...
          commission <- temp_commission + abs(balance_NEOBTC * m[i,15] * 0.002) +
            abs(balance_NEOUSD * m[i,12] * 0.002) + abs(balance_BTCUSD * m[i,3] * 0.002)
          temp_commission <- abs(balance_NEOBTC * m[i,15] * 0.002) + 
            abs(balance_NEOUSD * m[i,12] * 0.002) + abs(balance_BTCUSD * m[i,3] * 0.002)
        ...
        cat("PL=",format(round((PL - commission)/balance_used*100,2), nsmall = 2),
            "max_pl=",format(round(max_pl,2), nsmall = 2),
            "min_pl=",format(round(min_pl,2), nsmall = 2),"\n","\n")
      ...
      if(BuyNEOUSD == 1){
        ...
        if((PL>cond_close || PL<stop_loss || m[i,1]-start > stop_loss_dur
          ...
          if(BTCUSD_cond == 0){
            ...
            commission <- temp_commission + abs(balance_NEOBTC * m[i,15] * 0.002) +
              abs(balance_NEOUSD * m[i,12] * 0.002) + abs(balance_BTCUSD * m[i,4] * 0.002)
            temp_commission <- abs(balance_NEOBTC * m[i,15] * 0.002) + 
              abs(balance_NEOUSD * m[i,12] * 0.002) + abs(balance_BTCUSD * m[i,4] * 0.002)
...
x <- c("cond_open", "cond_close","trail_range","profit", "deals")
colnames(res) <- x
for(cond_open in seq(-2.0,0.2,by=0.1)){
  for(cond_close in seq(0.1,2.0,by=0.1)){
    for(trail_range in seq(0.1,1.0,by=0.1)){
    ...
    for(i in seq(11, nrow(m), by = 5)){
      ...
      if(trade == 0){
        ...
        if(BuyNEOBTC == 1){
          ...
          if((PL>cond_close && PL<trail || PL<stop_loss || m[i,1]-start > stop_loss_dur
              || i==nrow(m)-4) && trade_positive_difference == 1){
            #CLOSE 1:
            ##BUY NEO/USD && SELL NEO/BTC/USD
            temp_commission <- commission
            ...
            # cat("PL=",format(round((PL - commission)/balance_used*100,2), nsmall = 2),
            #     "max_pl=",format(round(max_pl,2), nsmall = 2),
            #     "trail=",format(round(trail,2), nsmall = 2),"\n","\n")
    ...
    t2 <- Sys.time()
    t2-t1
    cat("open=",cond_open,"close=",cond_close,"trail_range=",trail_range,
        "Total profit percent:", format(round((balance_usd/4000-1)*100,2), nsmall = 2),
        "% Total deals:",length(profit_sum),"time difference of",t2-t1,"secs\n")
    df <- data.frame(cond_open,cond_close,trail_range,format(round((balance_usd/4000-1)*100,2), nsmall = 2),length(profit_sum))
    x <- c("cond_open", "cond_close","trail_range","profit", "deals")
    colnames(df) <- x
    res <- rbind(res,df)
    }
