  ...
  if(trade == 0){
    if(TRUE){
      if(m[i,17] > hb){#min(m[i,17],m[i-5,17],m[i-10,17])
        ...
        #SELL NEO/USD && BUY NEO/BTC && BUY BTC/USD
        ...
        start <- m[i,1]
        s <- m[i,17]
        cat("START. SELL NEO/USD && BUY NEO/BTC && BUY BTC/USD | DEAL=",format(round(deal,2), nsmall = 2),"\n")
        ...
        if(m[i,18] < lb){
          ...
          #BUY NEO/USD && SELL NEO/BTC && SELL BTC/USD
          deal <- balance_avail * step_entry
          commission <- deal * 0.002 * 3
          start_balance <- balance_usd
          balance_usd <- start_balance - commission
...
write.csv(resH, "NEO_ResH.csv", row.names = FALSE)
