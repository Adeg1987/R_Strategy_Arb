  ...
  if(trade == 0){
    ...
    if(m[i,17] > s + scaling && trade_positive_difference == 1 && balance_avail > 100){
      ...
      avg_price_NEOBTC <- abs((temp_price_NEOBTC*temp_NEOBTC + deal/m[i,16]*m[i,8]) /
        ...
        #(balance_used / 2) / abs(balance_NEOBTC)
      
      s <- m[i,17]
      
      # cat("DateTime: ", as.character(as.POSIXct(m[i,1], origin = "1970-01-01")),
      #     "Balance USD:",format(round(temp_usd,2), nsmall = 2),
      #     "Balance NEO/BTC:",format(round(temp_NEOBTC,2), nsmall = 2),
      #     "Balance NEO/USD:",format(round(temp_NEOUSD,2), nsmall = 2),"\n")
      ...
      if(m[i,18] < s - scaling && trade_negative_difference == 1 && balance_avail > 100){
        ...
        avg_price_NEOUSD <- (balance_used / 2) / abs(balance_NEOUSD)
        temp_price_NEOBTC <- avg_price_NEOBTC
        avg_price_NEOBTC <- abs((temp_price_NEOBTC*temp_NEOBTC + deal/m[i,15]*m[i,7]) /
                                  (temp_NEOBTC + deal/m[i,15]))
...
x <- c("se", "sc", "scal","profit", "deals")
colnames(res) <- x
for(step_entry in seq(0.1,0.8,by=0.05)){
  for(step_continue in seq(0.1,0.8,by=0.05)){
    for(scaling in seq(1.1,2.0,by=0.05)){
    ...
    t2 <- Sys.time()
    cat("se=",step_entry,"sc=",step_continue,"scaling=",scaling,"Total profit percent:", format(round((balance_usd/4000-1)*100,2), nsmall = 2),
        "% Total deals:",length(profit_sum),"time difference of",t2-t1,"secs\n")
    df <- data.frame(step_entry,step_continue,scaling,format(round((balance_usd/4000-1)*100,2), nsmall = 2),length(profit_sum))
    x <- c("se", "sc", "scal","profit", "deals")
    colnames(df) <- x
    res <- rbind(res,df)
  }
...
write.csv(res, "NEO_balance.csv", row.names = FALSE)
    ...
    for(i in seq(11, nrow(m), by = 5)){
      if(trade == 0){
        ...
        if(m[i,17] > s + scaling && trade_positive_difference == 1 && balance_avail > 100){
          ...
          # cat("DateTime: ", as.character(as.POSIXct(m[i,1], origin = "1970-01-01")),
          #     "Balance USD:",format(round(temp_usd,2), nsmall = 2),
          #     "Balance NEO/BTC:",format(round(temp_btcNEO,2), nsmall = 2),
          #     "Balance NEO/USD:",format(round(temp_usdNEO,2), nsmall = 2),"\n")
          # cat("NEO/USD bid:", format(round(m[i,11],2), nsmall = 2),
          #     "NEO/USD ask:", format(round(m[i,12],2), nsmall = 2),
          #     "| NEO/BTC/USD bid:", format(round(m[i,15],2), nsmall = 2),
          #     "NEO/BTC/USD ask:", format(round(m[i,16],2), nsmall = 2),
          ...
          # cat("SELL NEO/USD && BUY NEO/BTC | DEAL=",format(round(deal,2), nsmall = 2),"\n")
          # cat("Balance USD:", format(round(balance_usd,2), nsmall = 2),
          #     "Balance NEO/BTC:", format(round(balance_btcNEO,2), nsmall = 2),
          #     "Balance NEO/USD:", format(round(balance_usdNEO,2), nsmall = 2),
          ...
          if(m[i,18] < s - scaling && trade_negative_difference == 1 && balance_avail > 100){
            ...
            if(m[i,18] < hn && trade_positive_difference == 1 || i == nrow(m)-4 && trade_positive_difference == 1){
              ...
              PL_usdNEO <- (m[i,12] - avg_price_usdNEO)*balance_usdNEO
              #if last row and then we close position and st PL=0
              if(i == nrow(m)-4){
                PL_btcNEO <- 0
                PL_usdNEO <- 0
              ...
              temp_commission <- abs(balance_btcNEO * m[i,16] * 0.002) +
                abs(balance_usdNEO * m[i,11] * 0.002)
              PL_btcNEO <- (m[i,16] - avg_price_btcNEO)*balance_btcNEO
...
write.csv(resL, "NEO_ResL.csv", row.names = FALSE)
