...
for(i in seq(11, nrow(m), by = 5)){
  ...
  if(trade == 0){
    ...
    if(BuyNEOBTC == 1){
      ...
      if(((PL_NEOBTC+PL_NEOUSD+PL_BTCUSD-commission*2)/balance_used*100>cond_close
          || (PL_NEOBTC+PL_NEOUSD+PL_BTCUSD-commission*2)/balance_used*100<stop_loss
...
colnames(res) <- x
for(cond_open in seq(-2.0,2.0,by=0.1)){
  for(cond_close in seq(0.1,3.0,by=0.1)){
    for(stop_loss in seq(-10,-100,by=-10)){
