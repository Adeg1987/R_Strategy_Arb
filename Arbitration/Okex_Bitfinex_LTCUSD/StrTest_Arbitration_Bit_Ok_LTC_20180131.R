...
BITF <- read.csv("BITF_LTCUSD_20170712_20180128.csv", header = TRUE)

setwd("C:/btc/Tics/Okex/OKFT_LTCUSD3M")
OKFT <- read.csv("OKFT_LTCUSD3M_20161231_20180105.csv", header = TRUE)

ts <- data.frame("Date" = 1499817600:1515110400)
...
acc_b_usd <- 4000
acc_o_usd <- 4000
bal_LTC_o <- acc_o_usd / m[1,3]
order_LTC_o <- 0
PL_o <- 0
hedge1 <- (bal_LTC_o * m[1,3]) %/% 100
hedge2 <- 0
acc_step_entry <- 0.1
  ...
  if(trade == 0){
    if(TRUE){
      if(m[i,4] > hb){
        ...
        acc_b_LTC <- c(acc_b_LTC, deal_b / (m[i,2]*sl))
        ...
        acc_o_LTC <- c(acc_o_LTC, -deal_o / (m[i,3]/sl))
        temp <- 0
        ...
        cat("Account Bitfinex USD:", format(round(acc_b_usd,2), nsmall = 2),
            ...
            "Account Bitfinex LTC:", format(round(sum(acc_b_LTC),2), nsmall = 2),
            "Account Okex LTC:", format(round(sum(acc_o_LTC),2), nsmall = 2),"\n")
        
        hedge2 <- deal_o / 100
        order_LTC_o <- hedge2 * 100 / m[i,3]
    ...
    bal_LTC_o <- hedge1 * 100 / m[i,3]
    temp <- order_LTC_o
    order_LTC_o <- hedge2 * 100 / m[i,3]
    if(trade_positive_difference){
      PL_o <- order_LTC_o - temp
    }
    else{
      PL_o <- temp - order_LTC_o
    }
    

    if(abs(price_o / m[i,3] - 1) > 0.05 && abs(PL_o * m[i,3]) > 100){ #abs(PL_o) / bal_LTC_o > 0.03
      cat("LTC balance=",format(round(bal_LTC_o,4), nsmall = 4),
          "LTC order=",format(round(order_LTC_o,4), nsmall = 4),"\n")
      temp <- bal_LTC_o
      bal_LTC_o <- temp + PL_o
      hedge1 <- (bal_LTC_o * m[i,3]) / 100
      cat("HEDGE!!!","Bal_LTC=",format(round(bal_LTC_o,4), nsmall = 4),
    ...
    if(m[i,4] > diff * 1.096 && trade_positive_difference == 1 &&
       abs(sum(acc_o_LTC)) * m[i,3] < hedge1 * 100){
      min_acc <- min(acc_b_usd, acc_o_usd)
      ...
      if(m[i,4] < diff * 1.096 && trade_negative_difference == 1){
        ...
        if(m[i,4] < hn && trade_positive_difference == 1 || i == nrow(m) && trade_positive_difference == 1){#Zakrivaem sdelki: pokupaem futures i prodaem bitfinex
          temp <- acc_b_usd
          acc_b_usd <- temp + sum(acc_b_LTC) * (m[i,2]/sl) - abs(sum(acc_b_LTC)) * m[i,2] * 0.002
          acc_b_LTC <- c()
          temp <- acc_o_usd
          acc_o_usd <- temp + sum(acc_o_LTC) * (m[i,3]*sl) - abs(sum(acc_o_LTC)) * m[i,3] * 0.0015
          acc_o_LTC <- c()
          temp <- 0
          hedge2 <- 0
          order_LTC_o <- 0
          bal_LTC_o <- acc_o_usd / m[i,3]
          PL_o <- 0
          hedge1 <- acc_o_usd / 100
          
          comm_b <- c(comm_b, acc_b_LTC * m[i,2] * 0.002)
          comm_o <- c(comm_o, acc_o_LTC * m[i,3] * 0.0015)
