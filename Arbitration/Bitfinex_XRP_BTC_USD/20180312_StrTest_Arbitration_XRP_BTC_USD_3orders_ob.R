  ...
  if(trade == 0){
    if(TRUE){
      if(m[i,17] > hb){#min(m[i,17],m[i-5,17],m[i-10,17])
        ...
        #SELL XRP/USD && BUY XRP/BTC && BUY BTC/USD
        ...
        start <- m[i,1]
        s <- m[i,17]
        cat("START. SELL XRP/USD && BUY XRP/BTC && BUY BTC/USD | DEAL=",format(round(deal,2), nsmall = 2),"\n")
        ...
        if(m[i,18] < lb){
          ...
          #BUY XRP/USD && SELL XRP/BTC && SELL BTC/USD
          deal <- balance_avail * step_entry
          commission <- deal * 0.002 * 3
          start_balance <- balance_usd
          balance_usd <- start_balance - commission
    ...
    if(m[i,17] > s + scaling && trade_positive_difference == 1 && balance_avail > 100){
      ...
      avg_price_XRPBTC <- abs((temp_price_XRPBTC*temp_XRPBTC + deal/m[i,16]*m[i,8]) /
        ...
        #(balance_used / 2) / abs(balance_XRPBTC)
      avg_price_BTCUSD <- (balance_used / 3) / abs(balance_BTCUSD)
      
      s <- m[i,17]
      
      # cat("DateTime: ", as.character(as.POSIXct(m[i,1], origin = "1970-01-01")),
      #     "Balance USD:",format(round(temp_usd,2), nsmall = 2),
      #     "Balance XRP/BTC:",format(round(temp_XRPBTC,2), nsmall = 2),
      #     "Balance XRP/USD:",format(round(temp_XRPUSD,2), nsmall = 2),"\n")
        ...
        if(m[i,18] < hn && trade_positive_difference == 1 || i == nrow(m)-4 && trade_positive_difference == 1){
          ...
          commission <- temp_commission + abs(balance_XRPBTC * m[i,15] * 0.002) +
            abs(balance_XRPUSD * m[i,12] * 0.002 +
                  abs(balance_BTCUSD * m[i,3] * 0.002))
...
colnames(resH) <- x
for(hb in seq(-5.0,5.0,by=0.2)){
  for(hn in seq(hb-5,hb-0.5,by=0.2)){
...
colnames(resL) <- x
for(lb in seq(-5.0,5.0,by=0.2)){
  for(ln in seq(lb+0.5,lb+5,by=0.2)){
...
setwd("C:/btc/Strategy/Arbitration/Bitfinex_XRP_BTC_USD")
write.csv(res, "XRP_balance.csv", row.names = FALSE)
