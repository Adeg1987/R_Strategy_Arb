...
BITF <- subset(BITF, select = -c(V1, V2, V3, V4, V6, V7, V9))
BITF <- aggregate(V5 ~ V8, data = BITF, mean)
BITF$V8 <- as.POSIXct(BITF$V8, origin="1960-01-01", tz="GMT")
...
OKFT <- read.csv("1075_OKFT_BTCUSD3M_20170807_20171128.csv", header = TRUE)
...
ts <- data.frame("Date" = 1486598400:1508198400)
ts$Date <- as.POSIXct(ts$Date, origin="1970-01-01", tz="GMT")

BITF_OK <- merge(x = ts, y = BITF, by.x = "Date", by.y = "V8", all.x = TRUE)
...
for(i in 20:nrow(m)){
  if(trade == 0){
    if(m[i,8] < min(m[i-1,8], m[i-2,8], m[i-3,8], m[i-4,8])){
      if(m[i,4] < -5.2){
        trade <- 1
    ...
    if(m[i,4] > -3.0){
      #Zakrivaem sdelki: pokupaem futures i prodaem bitfinex
      profit_bitf <- -m[i,2] * 10 + start_bitf
      profit_okft <- -start_okft + m[i,3] * 10
      profit <- c(profit, profit_bitf + profit_okft)
      cat("End deal at:", as.character(as.POSIXct(m[i,1], origin = "1970-01-1")), "\n",
          "StartBITF:", format(round(start_bitf,0), nsmall = 0),
          "StartOKFT:", format(round(start_okft,0), nsmall = 0),
          "CloseBITF:", format(round(m[i,2] * 10,0), nsmall = 0),
          "CloseOKFT:", format(round(m[i,3] * 10,0), nsmall = 0),
          "ProfitBITF:", format(round(profit_bitf,0), nsmall = 0),
          "ProfitOKFT:", format(round(profit_okft,0), nsmall = 0),
          "Profit:", format(round(profit_bitf + profit_okft,0), nsmall = 0), "\n")
      trade <- 0
      start_bitf <- 0
      start_okft <- 0
    }
